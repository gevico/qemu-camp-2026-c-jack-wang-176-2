...
static uint16_t be16_to_cpu(uint16_t be) {
    const uint8_t *ptr = (const uint8_t*)&be;
...
int main(void) {
    ...
    uint8_t real_version = version & 0xFF;
    unsigned major = (real_version>>4)& 0xF;
    unsigned minor = real_version & 0xF;
